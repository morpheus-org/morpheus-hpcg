...
void MorpheusInitializeVector(Vector& v) {
  v.optimizationData = new HPCG_Morpheus_Vec();
}

void MorpheusOptimizeSparseMatrix(SparseMatrix& A) {
  typename Morpheus::Csr::HostMirror Acsr("A", A.localNumberOfRows,
...
void MorpheusOptimizeVector(Vector& v) {
  using mirror            = typename Morpheus::UnmanagedVector::HostMirror;
  HPCG_Morpheus_Vec* vopt = (HPCG_Morpheus_Vec*)v.optimizationData;
